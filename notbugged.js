...
let globalData; // Déclarez une variable globale pour stocker les données

function getNames() {
  return fetch('getName.php')
    .then((response) => {
      if (!response.ok) {
    ...
    .then((data) => {
      globalData = data; // Assignez les données à la variable globale
      return data;
    })
    .catch((error) => {
...
// Utilisation de la fonction getNames
getNames()
  .then((namesData) => {
    // Vous pouvez également accéder aux données via la variable globale ici
